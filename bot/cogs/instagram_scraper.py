...
# ── Reintentos y rate limiting (MEJORA) ───────────────────────────────────────
try:
    ...
    log_warning(
        "[InstaShift] tenacity/aiolimiter no instalados. "
        "Para habilitar reintentos automáticos y rate limiting, ejecuta: "
        "pip install tenacity>=8.2.3 aiolimiter>=1.1.0"
    )
...
class InstagramClient:
    ...
    def __init__(self) -> None:
        ...
        # MEJORA: Rate limiting asincrónico (30 requests/minuto a Instagram)
        # Esto evita bloqueos de Instagram por demasiadas requests
        self._limiter: Optional[AsyncLimiter] = None
        if IMPROVEMENTS_AVAILABLE:
            self._limiter = AsyncLimiter(max_rate=30, time_period=60)
            log.info("[IG] ✅ Rate limiting habilitado (30 req/min)")
    ...
    async def get_user_info(self, username: str) -> Optional[object]:
        ...
        MEJORADO: Incluye rate limiting y reintentos automáticos.
        """
        if not INSTAGRAPI_OK or self._cl is None:
            return None
        ...
        try:
            # MEJORA: Rate limiting asincrónico
            if self._limiter:
                async with self._limiter:
                    # MEJORA: Reintentos automáticos
                    if IMPROVEMENTS_AVAILABLE:
                        async for attempt in AsyncRetrying(
                            retry=retry_if_exception_type(UserNotFound),
    ...
    async def get_recent_medias(self, username: str, amount: int = 5) -> list:
        ...
        MEJORADO: Incluye rate limiting y reintentos automáticos con backoff exponencial.
        """
        if not INSTAGRAPI_OK or self._cl is None:
            return []
        await self.ensure_logged_in()
        ...
        try:
            ...
            if self._limiter:
                async with self._limiter:
                    ...
                    # MEJORA: Reintentos automáticos con tenacity
                    if IMPROVEMENTS_AVAILABLE:
                        async for attempt in AsyncRetrying(
                            retry=retry_if_exception_type((UserNotFound, MediaNotFound)),
                        ...
                        # Sin mejoras: comportamiento original
                        user_id = await loop.run_in_executor(
                            None, lambda: self._cl.user_id_from_username(username)
                        )
                        medias = await loop.run_in_executor(
                    ...
                    log.info(f"[IG] ✓ {len(medias)} medias obtenidas de @{username} en {duration:.2f}s")
                    return medias
            else:
                # Fallback si rate limiter no está disponible
                user_id = await loop.run_in_executor(
                    None, lambda: self._cl.user_id_from_username(username)
                )
                medias = await loop.run_in_executor(
