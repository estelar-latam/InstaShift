...
# MEJORA: Importar sistema de monitoreo (Prometheus + Sentry)
try:
    from bot.utils.monitoring import health_check, metrics
    MONITORING_AVAILABLE = True
except ImportError:
    MONITORING_AVAILABLE = False
    log_temp = logging.getLogger(__name__)
    log_temp.warning(
        "[InstaShift] prometheus-client no está instalado. "
        "Para habilitar métricas Prometheus, ejecuta: pip install prometheus-client>=0.18.0"
    )
...
# MEJORA: Inicializar Sentry para error tracking (si está configurado)
...
if _sentry_dsn:
    try:
        ...
        del _sentry_dsn  # Limpiar variable temporal
    except ImportError:
        pass  # sentry_sdk no instalado
...
class InstaShift(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        # MEJORA: Registrar health checks (si monitoreo está disponible)
        if MONITORING_AVAILABLE:
            log.info("[Monitoring] Registrando health checks...")

            # Health check: Instagram session
            async def check_instagram_session() -> bool:
                """Verifica que la sesión de Instagram sea válida."""
                try:
                    from bot.cogs.instagram_scraper import instagram_client
                    return instagram_client.is_authenticated
                except Exception:
                    return False
            ...
            # Health check: Cogs cargados
            async def check_cogs() -> bool:
                """Verifica que todos los cogs estén cargados."""
                return cogs_fallados == 0
            ...
            log.info("[Monitoring] ✅ Health checks registrados")
